...
public class SearchThread extends Thread {
	...
	public void run() {
		...
		try {
			...
			for(int i=0; i<perPageProduct.size(); i++){
				list.add(perPageProduct.get(i).select("ul.xans-element- span:eq(0)"));			//한 상품에 해당하는 태그들(Element) 중에서 ul태그의 클래스명이  "xans-element-"이면서 그 자손태그들 중에 첫 번째 span태그를 가져옴.
	...
	/*public static void main(String[] args) {
		...
		SearchThread thread = new SearchThread("http://66girls.co.kr/product/search.html?banner_action=&keyword=바지&page=2");
		thread.run();
		}
	}*/
